...
const Product = ({ btn, btn1 }) => {
  ...
  useEffect(() => {
    ...
    if (id) {
      axios.get(`${API_URL}/products/${id}`).then((ris) => setData(ris.data));
    }
  }, [searchParms]);
  ...
  let cards = data?.products?.map((product) => (
    <div className="card">
      <img
        onClick={() => setSearchParms({ details: product.id })}
        width={200}
  ...
  return (
    <div>
      <div className="container">
          ...
          {category?.map((el) => (
            <li key={el}>
              <data
                style={{
                  background:
                    categoryAll === `/category/${el}` ? "#F7Eb95" : " #F7EBE5",
                }}
        ...
        {isLoading ? <Loading /> : <></>}
        <div className="wraper">{cards}</div>
        <div className="btn1">
          <button onClick={() => setCount((p) => p + 1)} className="btn">
            Показать ещё
          </button>
        </div>
      </div>
